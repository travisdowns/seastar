...
SEASTAR_THREAD_TEST_CASE(test_dn_name_handling) {
    ...
    auto client1_creds = [] {
        ...
        builder.set_x509_trust_file("mtls_ca.crt", tls::x509_crt_format::PEM).get();
        builder.set_x509_key_file("mtls_client1.crt", "mtls_client1.key", tls::x509_crt_format::PEM).get();
    ...
    auto client2_creds = [] {
        tls::credentials_builder builder;
        builder.set_x509_trust_file("mtls_ca.crt", tls::x509_crt_format::PEM).get();
        builder.set_x509_key_file("mtls_client2.crt", "mtls_client2.key", tls::x509_crt_format::PEM).get();
        return builder.build_certificate_credentials();
    }();
    ...
    auto server_creds = [] {
        tls::credentials_builder builder;
        builder.set_x509_trust_file("mtls_ca.crt", tls::x509_crt_format::PEM).get();
        builder.set_x509_key_file("mtls_server.crt", "mtls_server.key", tls::x509_crt_format::PEM).get();
    ...
    auto ssock = tls::listen(server_creds, addr, lo, true);
    ...
    (void)with_gate(accept_gate, [expected_accepts, ssock=std::move(ssock)] () mutable {
        return async([expected_accepts, ssock=std::move(ssock)] () mutable {
            while (expected_accepts --> 0) {
                ...
                auto fdn = connection.get_distinguished_name();
