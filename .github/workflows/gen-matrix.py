...
appear at all (see CONSTRAINTS) are the exception, and
_validate_pairwise() enforces the guarantee over every other pair. The
file is round-tripped through ruamel.yaml, replacing only the strategy
matrix include list; everything else in tests.yaml is left untouched.
